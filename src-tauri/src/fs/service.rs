...
impl FilesystemService {
    ...
    pub fn new() -> Self {
        Self { base_path: "/home/sander/Dokumente/CloudLite".to_owned() }
    }
    ...
    fn localize_user_path(&self, user_absolute_path: &str, destination_path: Option<&str>) -> PathBuf {
    ...
    pub async fn write_dropped_paths(&self, paths: Vec<String>, destination_path: Option<String>) -> Result<(), String> {
                    ...
                    if let Ok(stripped) = entry.strip_prefix(parent) {
                        if entry.is_dir() {
                            let directory = PathBuf::from(&self.base_path)
                                ...
                                .join(stripped);
                            self.create_directory(directory).expect("Failed to create directory");
                        }
                        // ToDo: Error handling
                        if entry.is_file() {
